...
from judge.tasks import compare_the_same_points
...
def SubmissionSourceDiff(request):
    ...
    first_source = first_sub.source.source
    second_source = second_sub.source.source
    plagiarism_similarity = compare_the_same_points.calculate_similarity(first_source, second_source)
    ...
    return render(request, 'submission/source-diff.html', {
        ...
        'plagiarism_similarity': plagiarism_similarity,
    })
...
def single_submission(request):
    ...
    # point = cal_sub_id(submission_id, submission.source.source)
...
@login_required
def SubmissionCheck(request):
    point = []
    max_id = []

    submissions = get_list_or_404(Submission)

    for first_submission in submissions:
        check = 0
        max_sub_id = 0
        for compare_submission in submissions:
            if first_submission != compare_submission:
                temp = compare_the_same_points.calculate_similarity(first_submission.source.source,
                                                                    compare_submission.source.source)
                if temp > check:
                    check = temp
                    max_sub_id = compare_submission.id

        point.append(check)
        max_id.append(max_sub_id)
    check_sub = get_object_or_404(Submission, id=max_sub_id)
    name_check = check_sub.user.username
    tilte = 'Check plagiarism'



    return render(request, "submission/check-plagiarism.html", {
        'title': tilte,
        'submissions': submissions,
        'point': point,
        "max_id": max_id,
        'problem_name': submissions[0].problem.translated_name(request.LANGUAGE_CODE)
